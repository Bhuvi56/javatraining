...
public class ProductService {
	...
	public void usingTxnWithCatchBlock(Product prd1,Invoice invoice) {
		String addProductsql = "insert into  bhu_product values(?,?,?)";
		String addInvoicesql = "insert into  bhu_invoice values(?,?,?,?)";
		
		try(PreparedStatement prodStmt= con.prepareStatement(addProductsql);
				PreparedStatement invoicePstmt= con.prepareStatement(addInvoicesql)){
			...
			invoicePstmt.setString(2,invoice.getCustomerName());
			invoicePstmt.setDouble(3, invoice.getQuantity());
